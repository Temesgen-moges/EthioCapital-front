...
import FundReleaseManagement from './FundReleaseManagement'; // New import
import {
...
const AdminDashboard = () => {
  ...
  const [fundReleaseCount, setFundReleaseCount] = useState(0); // New state for fund release count
  ...
  useEffect(() => {
    const fetchFundReleaseCount = async () => {
      try {
        ...
        const response = await axios.get('http://localhost:3001/api/v1/fund-release/status/sent_to_admin', {
          headers: { Authorization: `Bearer ${token}` },
        });
        setFundReleaseCount(response.data.data?.length || 0);
      } catch (error) {
        console.error('[AdminDashboard] fetchFundReleaseCount error:', error);
      }
    };
  ...
  const fetchPendingIdeas = async () => {
    ...
    try {
      const token = localStorage.getItem('authToken');
      const url = 'http://localhost:3001/api/v1/get-pending-ideas';
      console.log('Requesting URL:', url, 'Token:', token);
      ...
      if (error.response) {
        if (error.response.status === 401) errorMsg = 'Authentication failed. Please log in again.';
        else if (error.response.status === 403) errorMsg = 'You do not have permission to view pending ideas.';
        else if (error.response.status === 404) errorMsg = 'Ideas endpoint not found. Check server configuration.';
        else if (error.response.status >= 500) errorMsg = 'Server error. Please try again later or contact support.';
        else errorMsg = error.response.data?.message || errorMsg;
  ...
  const refreshData = () => {
    ...
    const refreshIdeas = async () => {
      try {
        const token = localStorage.getItem('authToken');
        const url = 'http://localhost:3001/api/v1/get-pending-ideas';
        console.log('Refreshing URL:', url, 'Token:', token);
        const response = await axios.get(url, {
          headers: {
            Authorization: `Bearer ${token}`,
          },
        ...
        console.error('Error refreshing ideas:', {
          message: error.message,
          status: error.response?.status,
          data: error.response?.data,
          url: error.config?.url,
        });
        let errorMsg = 'Failed to refresh ideas. Please try again.';
        if (error.response) {
          if (error.response.status === 401) errorMsg = 'Authentication failed. Please log in again.';
          else if (error.response.status === 403) errorMsg = 'You do not have permission to view pending ideas.';
          else if (error.response.status === 404) errorMsg = 'Ideas endpoint not found. Check server configuration.';
  ...
  const handleStatusChange = async (ideaId, approvalStatus, ranking) => {
    try {
      await axios.put(`http://localhost:3001/api/v1/approve-idea/${ideaId}`, { approvalStatus, ranking }, {
        headers: {
      ...
      setIdeas((prevIdeas) =>
        prevIdeas.map((idea) =>
          idea._id === ideaId ? { ...idea, approvalStatus, ranking } : idea
        )
      );
      setSelectedIdea(null);
    } catch (error) {
      console.error('Error updating idea status:', error.response || error.message);
      setErrorMessage('Failed to update idea status. Please try again.');
    }
  };
          ...
          <div className="flex-grow pt-5 px-3">
            <ul className="space-y-2">
              {[
                ...
                { name: 'Fund Releases', icon: DollarSign, action: () => setActiveTab('fund releases') }, // New sidebar item
