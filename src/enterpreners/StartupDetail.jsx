  ...
  {userData.userData?._id ? (
    hasDocumentAccess || isEntrepreneur ? (
      <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
        ...
        {[
          { name: "Business Registration", path: ideaDetails.businessRegistration },
          { name: "Financial Projections", path: ideaDetails.financialProjections },
          { name: "Market Research Report", path: ideaDetails.marketResearchReport },
          { name: "Patent Documentation", path: ideaDetails.patentDocumentation },
          { name: "Pitch Deck", path: ideaDetails.pitchDeck },
          { name: "Team Certifications", path: ideaDetails.teamCertifications },
        ].filter(doc => {
          console.log(`Document ${doc.name}:`, doc.path); // Debug each document
          ...
          [
            ...
            { name: "Pitch Deck", path: ideaDetails.pitchDeck },
            { name: "Team Certifications", path: ideaDetails.teamCertifications },
          ].filter(doc => doc.path && typeof doc.path === 'string' && doc.path.trim() !== '').map((doc, index) => {
            if (!doc.path.toLowerCase().endsWith('.pdf')) {
              return (
                ...
                >
                  <p className="text-red-600">Only PDF documents are supported for {doc.name}</p>
                </div>
              );
            }
            ...
            const handleDocumentClick = async () => {
              ...
              try {
                ...
                if (response.status === 200) {
                  ...
                  console.log('Opening document viewer at:', `/view-document/${encodedPath}`);
                  window.open(`/view-document/${encodedPath}`, '_blank');
                } else {
                  setDocumentError(`Document "${doc.name}" not found. Please contact support.`);
                }
              } catch (error) {
                console.error('Error checking document:', error.response?.status, error.message);
                setDocumentError(`Error accessing "${doc.name}": ${error.message}`);
              }
            };
