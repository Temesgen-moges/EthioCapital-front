...
useEffect(() => {
  if (!fabricCanvasRef.current) {
    fabricCanvasRef.current = new Canvas('captureCanvas', {
      width: 640,
      height: 480,
    });
  }
 ...
 const captureImage = () => {
  if (videoRef.current && fabricCanvasRef.current) {
    const video = videoRef.current;
    fabricCanvasRef.current.setWidth(video.videoWidth);
    fabricCanvasRef.current.setHeight(video.videoHeight);

    const image = new Image(video, {
      left: 0,
      top: 0,
    ...
    fabricCanvasRef.current.toBlob((blob) => {
      const imageFile = new File([blob], "profile-picture.png", {
        type: "image/png",
      });
      setFormData((prev) => ({ ...prev, profilePicture: imageFile }));
            ...
            {!showPreview ? (
              <div className="relative mb-6 rounded-xl overflow-hidden">
                <video
                  ...
                  playsInline
                />
                <canvas id="captureCanvas" className="hidden" />
                <button
                  onClick={toggleCameraFacing}
                  className="absolute top-4 right-4 bg-white bg-opacity-80 p-2 rounded-full hover:bg-opacity-100 transition-all"
                  aria-label="Switch camera"
  ...
  "Request Access"
                ...
                <p className="text-gray-600">
                  Please log in to view or request access to documents.
                </p>
              )}
            </section>

            {/* Investment Details Section */}
            {userData.userData?.role === "investor" && data?.investments?.length > 0 && (
              <section className="bg-white p-6 rounded-2xl shadow-md">
                <h2 className="text-2xl font-semibold text-gray-900 mb-6">Your Investment Details</h2>
                <div className="space-y-4">
                  {data.investments
                    .filter((inv) => inv.email === userInfo.email)
                      ...
                      >
                        <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
                          ...
                          <div>
                            ...
                            <p className="text-lg font-semibold text-gray-900">
                              {(investment.equityPercentage * 100).toFixed(2)}%
                            </p>
                          </div>
                          <div>
          ...
          <div className="lg:col-span-1 space-y-8">
              ...
              <div className="space-y-4">
                <button
                  ...
                  disabled={
                    (userData.userData?.role === "investor" &&
                      verificationStatus !== "approved") ||
                    (isEntrepreneur && remainingFunding > 0)
                  }
                ...
                >
                  {remainingFunding <= 0 ? "View Board" : "Invest Now"}
                </button>
                {userData.userData?.role === "investor" &&
                  verificationStatus !== "approved" && (
                    <p className="text-sm text-gray-500 text-center">
                      {verificationStatus === "pending"
                        ...
                        : verificationStatus === "rejected"
                        ? "Verification rejected. Please update your information."
                        : "Verify your investor status to proceed."}
                    </p>
                  )}
                {isEntrepreneur && remainingFunding > 0 && (
                  <p className="text-sm text-gray-500 text-center">
                    You cannot invest in your own idea.
                  </p>
                )}
                <button
                  onClick={handleSaveIdea}
            ...
            <div className="bg-white p-6 rounded-2xl shadow-md">
              ...
              <div className="space-y-4">
                ...
                <div>
                  ...
                  <p className="text-base font-medium text-gray-900">
                    {ideaDetails.minimumInvestment
                      ? Number(ideaDetails.minimumInvestment).toLocaleString("en-US", {
                          style: "currency",
                          currency: "ETB",
                          minimumFractionDigits: 0,
                        })
            ...
            {/* Entrepreneur Contact (for investors) */}
            {userData.userData?.role === "investor" && (
              <div className="bg-white p-6 rounded-2xl shadow-md">
                <h2 className="text-xl font-semibold text-gray-900 mb-6">Contact Entrepreneur</h2>
                <div className="space-y-4">
                  <div>
                    <p className="text-sm text-gray-500">Name</p>
                    <p className="text-base font-medium text-gray-900">
                      {ideaDetails?.user?.fullName || "Not specified"}
                    </p>
                  </div>
                  <div>
                    <p className="text-sm text-gray-500">Email</p>
                    <p className="text-base font-medium text-gray-900">
                      {ideaDetails?.user?.email || "Not specified"}
                    </p>
                  </div>
                  <button
                    onClick={(e) =>
                  ...
                  {verificationStatus !== "approved" && (
                    <p className="text-sm text-gray-500 text-center">
                      Verify your investor status to contact the entrepreneur.
                    </p>
                  )}
                </div>
              </div>
