...
//   const STATIC_URL = "https://ethiocapital-back.onrender.com";
...
//       try {
//         const response = await axios.get(
//           `https://ethiocapital-back.onrender.com/api/v1/investments-details/${projectId}`
//         );
...
const StartupDetail = () => {
  ...
  const isEntrepreneur =
    userData.userData?.role === "entrepreneur" &&
    ideaDetails?.user?._id === userData.userData._id;
  const canEditOrDelete = isEntrepreneur && (data?.fundingRaised || 0) === 0;
  ...
  // Move fetchDocumentAccessRequests to a useEffect that triggers when showAccessRequests changes
useEffect(() => {
  if (isEntrepreneur && showAccessRequests) {
    fetchDocumentAccessRequests();
  }
}, [showAccessRequests, isEntrepreneur]);
  ...
  const fetchDocumentAccessRequests = async () => {
    ...
    try {
      ...
      const requests = response.data.requests || response.data || [];
      console.log("Document request data : ",requests);
  ...
  const handleDeleteIdea = async () => {
    ...
    try {
      await axios.delete(`${API_URL}/business-ideas/${id}`, {
        headers: { Authorization: `Bearer ${token}` }
      });
      ...
      setDeleteError(error.response?.data?.message || "Failed to delete idea. Please try again.");
    } finally {
      setIsDeleting(false);
    }
  };

  const handleRequestDetails = (requesterId) => {
  ...
  return (
    <div className="min-h-screen bg-gray-100 font-sans">
      ...
      {isEntrepreneur && (
        <div className="fixed top-4 right-4 z-50">
  <button
    onClick={() => setShowAccessRequests(!showAccessRequests)} // Only toggle the modal
    className="flex items-center gap-2 py-2 px-4 bg-blue-600 text-white rounded-xl shadow-md hover:bg-blue-700 transition-all"
  >
    <svg
      className="w-5 h-5"
                ...
                <div className="space-y-4">
                  {documentAccessRequests.map((request) => (
                    <div
                      ...
                      onClick={() => handleRequestDetails(request.requesterId)}
                    >
                      <div className="flex flex-col md:flex-row justify-between items-start md:items-center gap-4">
                        <div className="flex-1">
                          <div className="flex items-center gap-3">
  ...
  <div className="fixed inset-0 bg-black bg-opacity-60 flex items-center justify-center z-[80] p-4">
    <div className="bg-white rounded-2xl w-full max-w-3xl max-h-[80vh] overflow-y-auto shadow-2xl">
      <div className="p-6 border-b border-gray-200">
        ...
        <div className="flex items-center gap-3 mt-4">
          <button
            onClick={fetchDocumentAccessRequests} // Call fetchDocumentAccessRequests only here
            disabled={isRequestsLoading}
            className={`flex items-center gap-2 px-4 py-2 bg-blue-600 text-white rounded-xl hover:bg-blue-700 transition-all ${
              isRequestsLoading ? "opacity-50 cursor-not-allowed" : ""
            }`}
      ...
      <div className="p-6">
        {confirmationMessage && (
          <div className="mb-4 p-3 bg-blue-100 text-blue-800 rounded-xl text-center">
            {confirmationMessage}
          </div>
        )}
        {isRequestsLoading ? (
                ...
                <div className="flex flex-col md:flex-row justify-between items-start md:items-center gap-4">
                  <div className="flex-1 flex items-center gap-3">
                    <button
                      ...
                      aria-label={`View ${request.requesterName}'s profile`}
                    >
                      <svg
                      ...
                      >
                        <path
                          ...
                          strokeLinejoin="round"
                          strokeWidth={2}
                          d="M12 14c-3.86 0-7-3.14-7-7s3.14-7 7-7 7 3.14 7 7-3.14 7-7 7zm0 0c4.42 0 8 3.58 8 8v1H4v-1c0-4.42 3.58-8 8-8z"
                        />
                      </svg>
                    </button>
                    <div>
                  ...
                  {request.status === "pending" && (
                    <div className="flex gap-3">
                      <button
                        onClick={() =>
                          handleAccessRequestAction(request._id, "approve")
                        }
                        disabled={requestActionLoading[request._id]}
                        className={`flex items-center gap-2 px-4 py-2 bg-green-600 text-white rounded-xl hover:bg-green-700 transition-all ${
                          requestActionLoading[request._id]
      ...
      <div className="bg-white shadow-sm">
        <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-12">
          {/* Entrepreneur Controls (Edit/Delete) */}
          {canEditOrDelete && (
