...
const InvestorProfile = () => {
  ...
  const userId = localStorage.getItem("userId"); // Authenticated user
  const API_URL = "https://ethiocapital-back.onrender.com/api/v1";

  // Mock investor data for fallback
  const investor = {
    name: "John Maxwell",
  ...
  // Fetch access requests for the authenticated user
  useEffect(() => {
    if (!userId || !token) {
      console.log("Missing user ID or token:", { userId, token });
      return;
    }
    fetchAccessRequests();
  }, [userId, token]);
  ...
  const fetchAccessRequests = async () => {
    ...
    try {
      const response = await axios.get(`${API_URL}/document-access/requests/${userId}`, {
        headers: { Authorization: `Bearer ${token}` },
      });
      console.log("Access requests fetched:", response.data.requests);
  ...
  return (
    <div className="min-h-screen bg-gray-50">
      <div className="bg-blue-600 text-white py-4">
        <div className="max-w-6xl mx-auto px-4 flex items-center gap-4">
          ...
          <h1 className="text-xl font-bold">
            {isOwnProfile ? "My Profile" : "Investor Profile"}
          </h1>
        </div>
        <motion.button
      ...
      <div className="p-8">
        ...
        {loading ? (
          <div className="text-gray-600 text-center py-4">Loading profile...</div>
        ) : (
          <motion.div
          ...
          >
            <div className="bg-gradient-to-r from-blue-600 to-blue-800 p-8">
              ...
              >
                <div className="h-24 w-24 bg-white rounded-full flex items-center justify-center">
                  <img
                    src={profile?.photo || investor.photo || "https://via.placeholder.com/150"}
                    alt="Profile"
                    className="w-full h-full rounded-full object-cover"
                  />
                </div>
                <div className="text-white text-center md:text-left">
                  <h1 className="text-3xl font-bold">{profile?.fullName || investor.name}</h1>
                  <p className="text-blue-100">{profile?.title || investor.title}</p>
                  <div className="flex gap-4 mt-4 justify-center md:justify-start">
                    <a
                      href={profile?.socialLinks?.linkedin || investor.socialLinks.linkedin}
                      className="text-white hover:text-blue-200 transition-colors"
                    >
                      <Linkedin className="w-5 h-5" />
                    </a>
                    <a
                      href={profile?.socialLinks?.twitter || investor.socialLinks.twitter}
                      className="text-white hover:text-blue-200 transition-colors"
                    >
                      <Twitter className="w-5 h-5" />
                    </a>
                    <a
                      href={profile?.socialLinks?.website || investor.socialLinks.website}
                      className="text-white hover:text-blue-200 transition-colors"
                    >
                      <Globe className="w-5 h-5" />
                    </a>
                    <a
                      href={`mailto:${profile?.socialLinks?.email || investor.socialLinks.email}`}
                      className="text-white hover:text-blue-200 transition-colors"
                    >
                      <Mail className="w-5 h-5" />
                    </a>
                    <a
                      href={profile?.socialLinks?.instagram || investor.socialLinks.instagram}
                      className="text-white hover:text-blue-200 transition-colors"
                    >
            ...
            {/* Toggle Button for Document Requests */}
            <motion.div
              initial={{ opacity: 0, y: 20 }}
              animate={{ opacity: 1, y: 0 }}
            ...
            {/* Document Access Requests Section */}
            <AnimatePresence>
              {showRequests && (
                <motion.div
              ...
              <div className="space-y-6">
                ...
                >
                  ...
                  <div className="space-y-3">
                    <div className="flex items-center gap-2">
                      ...
                      <span>{profile?.capital || investor.capital}</span>
                    </div>
                    <div className="flex items-center gap-2">
                      <Briefcase className="text-blue-600" />
                      <span className="font-medium">Experience:</span>
                      <span>{profile?.experience || investor.experience}</span>
                    </div>
                    <div className="flex items-center gap-2">
                      <Trophy className="text-blue-600" />
                      <span className="font-medium">Successful Exits:</span>
                      <span>{profile?.successfulExits || investor.successfulExits}</span>
                    </div>
                  </div>
                </motion.div>
                ...
                >
                  ...
                  <div className="flex flex-wrap gap-2">
                    {(profile?.preferredFields || investor.preferredFields).map((field, index) => (
            ...
            >
              ...
              <div className="grid grid-cols-1 md:grid-cols-3 gap-6">
                {(profile?.previousInvestments || investor.previousInvestments).map((investment, index) => (
                  <motion.div
                    key={index}
            ...
            >
              ...
              <p className="text-gray-600 leading-relaxed">{profile?.about || investor.about}</p>
            </motion.div>
          </motion.div>
        )}
      </div>
